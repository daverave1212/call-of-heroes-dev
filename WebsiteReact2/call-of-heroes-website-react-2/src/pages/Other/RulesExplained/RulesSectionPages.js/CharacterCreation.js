...
const customInsertables = {
    'Initiative': () => (<SmallStat className="column" name="Initiative">Your Dexterity + Your Charisma</SmallStat>),
    'Stats': () => (
    ...
    'Max Health': () => (<SmallStat className="column" name="Max Health">Race Health + Your Might</SmallStat>),
    'Speed': () => (<SmallStat className="column" name="Movement Speed">Given by your Race</SmallStat>),
    ...
    'Speed': () => (<SmallStat className="column" name="Spell Grade">6 + A Specific Stat</SmallStat>),
}
            ...
            <div style={{marginTop: 'var(--page-padding)', whiteSpace: 'pre-wrap'}} className='with-margined-children'>
                ...
                {subtitles.map(subtitle => (
                    subtitlesThatAreSmallStats.includes(subtitle) == false ? (
                        ...
                        <div key={subtitle}>
                            <PageH3>{subtitle}</PageH3>
                            <SmallStat className="column" name={subtitle}>{cc[title][subtitle]}</SmallStat>
                        </div>
                    )
                ))}
            </div>
    ...
    return (
        <Page title="Character Creation" isSecondaryPage={isSecondaryPage}>
            ...
            <div className='with-margined-children'>
                ...
                <SmallStat name="Armor" className="column">{ cc['Stat Calculations']['Armor'] }</SmallStat>
                <SmallStat name="Weapon and Tool Training" className="column">{ cc['Stat Calculations']['Training'] }</SmallStat>
                <SmallStat name="Languages" className="column">{ cc['Stat Calculations']['Languages'] }</SmallStat>
            </div> */}
        </Page>
    )
